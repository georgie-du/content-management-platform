...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!isPasswordMatching) return res.status(400).json({ message: "Invalid password" });

    const token = jwt.sign({ email: currentUser.email, id: currentUser._id }, secret, { expiresIn: "2h" });
